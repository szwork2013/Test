...
//        	infoList = db.findAll(Selector.from(DownloadInfo.class));
    ...
    public DownloadInfo getDownloadInfoByCourseId(String courseId, long itemId) {
        if (downloadInfo4CourseList.isEmpty()) {
            return null;
        }
        for (DownloadInfo4Course info4Course : downloadInfo4CourseList) {
    ...
    public int getDownloadIngCount() {
        ...
        if (infoList != null) {
            for (DownloadInfo downloadInfo : infoList) {
                ...
                if (state.equals(HttpHandler.State.LOADING) || state.equals(HttpHandler.State.WAITING)) {
                    num++;
                }
            }
        }
    ...
    public int getDownloadPauseCount() {
        ...
        if (infoList != null) {
            for (DownloadInfo downloadInfo : infoList) {
                HttpHandler.State state = downloadInfo.getState();
                if (state.equals(HttpHandler.State.CANCELLED) || state.equals(HttpHandler.State.FAILURE)) {
                    num++;
                }
            }
        }
