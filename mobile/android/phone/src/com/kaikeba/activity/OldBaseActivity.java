...
public class OldBaseActivity extends Activity {
    ...
    protected void handleOutmemoryError() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(OldBaseActivity.this, "内存空间不足！", Toast.LENGTH_SHORT)
                        .show();
                // finish();
            }
        });
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (network_err_count < 3) {
                    Toast.makeText(OldBaseActivity.this, "网速好像不怎么给力啊！",
                            Toast.LENGTH_SHORT).show();
                } else if (network_err_count < 5) {
                    Toast.makeText(OldBaseActivity.this, "网速真的不给力！",
                            Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(OldBaseActivity.this, "唉，今天的网络怎么这么差劲！",
                            Toast.LENGTH_SHORT).show();
                }
                // finish();
            }
    ...
    protected void handleMalformError() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(OldBaseActivity.this, "数据格式错误！", Toast.LENGTH_SHORT)
                        .show();
                // finish();
            }
        });
    ...
    protected void handleFatalError() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(OldBaseActivity.this, "发生了一点意外，程序终止！",
                        Toast.LENGTH_SHORT).show();
                finish();
            }
        });
