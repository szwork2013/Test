...
- (void)presentFromViewController:(UIViewController *)viewController
                            ...
                            items:(NSArray *)items
                  formatCellBlock:(void (^)(UITableViewCell *cell,
                                            id item))formatCellBlock
                  isSelectedBlock:(BOOL (^)(id item))isSelectedBlock
               didSelectItemBlock:(void (^)(id item))didSelectItemBlock
                  didDismissBlock:(void (^)(void))didDissmissBlock {
    ...
    self.pickerView = [[UIView alloc]
        initWithFrame:CGRectOffset(viewController.view.bounds, 0, 0)];
    self.pickerView.clipsToBounds = YES;
    self.pickerView.autoresizingMask =
        UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [viewController.view addSubview:self.pickerView];

    self.overlay = [[UIView alloc]
        initWithFrame:CGRectSetOrigin(self.pickerView.frame, CGPointZero)];
    ...
    self.containerView = [[UIView alloc]
        initWithFrame:CGRectInset(
                          CGRectSetOrigin(self.pickerView.frame, CGPointZero),
    ...
    UINavigationBar *navigationBar = [[UINavigationBar alloc]
        initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.containerView.frame),
                                 navigationBarHeight)];
    ...
    navigationBar.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin |
    ...
    [closeButton setImage:[UIImage imageNamed:@"VKPickerButton_cross"]
                 forState:UIControlStateNormal];
    closeButton.frame =
        CGRectMake(0, 0, navigationBarHeight, navigationBarHeight);
    ...
    self.tableView = [[UITableView alloc]
        initWithFrame:CGRectMake(0, CGRectGetMaxY(navigationBar.frame),
    ...
    UIImageView *shadow = [[UIImageView alloc]
        initWithImage:[UIImage imageNamed:@"VKPickerButton_bg"]];
    shadow.frame = CGRectMake(0, navigationBarHeight,
                              CGRectGetWidth(navigationBar.frame), 8.0f);
    shadow.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin |
                              UIViewAutoresizingFlexibleWidth;
...
- (void)dismiss {
    [UIView animateWithDuration:0.3f
        animations:^{
            [self.containerView
                setFrameOriginY:CGRectGetHeight(self.pickerView.frame)];
            self.overlay.alpha = 0.0f;
        }
...
- (void)tableView:(UITableView *)tableView
    ...
    if (self.isSelectedBlock &&
        self.isSelectedBlock(self.items[indexPath.row])) {
