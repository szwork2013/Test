...
- (id)initWithRawString:(NSString *)subtitleRawData {
    ...
    if (self) {
        ...
        [self parseSubtitleRaw:subtitleRawData
                    completion:^(NSMutableArray *segments,
                                 NSMutableArray *invalidSegments) {
...
- (void)parseSubtitleRaw:(NSString *)string
              completion:(void (^)(NSMutableArray *segments,
                                   NSMutableArray *invalidSegments))completion {
    completion(nil, nil);
}
...
- (NSInteger)millisecondsFromTimecodeString:(NSString *)timecodeString {
    ...
    if (timeComponents.count > 2) {
        secondsComponents = [(NSString *)[timeComponents objectAtIndex:2]
            componentsSeparatedByString:@","];
        if (secondsComponents.count > 0)
            seconds =
                [(NSString *)[secondsComponents objectAtIndex:0] integerValue];
        if (secondsComponents.count > 1)
            milliseconds =
                [(NSString *)[secondsComponents objectAtIndex:1] integerValue];
    }
...
- (id)initWithAttributes:(NSDictionary *)attributes {
    ...
    if (!self)
        return nil;
    self.languageCode =
        [attributes valueForKeyPathWithNilCheck:@"language_code"];
...
- (NSNumber *)closestPreferredAdSlotTimeInMilliseconds:
                  (NSNumber *)timeInMilliseconds {
    if (self.preferredAdSlotTimes.count == 0)
        return nil;

    NSUInteger index = [self.preferredAdSlotTimes
          indexOfObject:timeInMilliseconds
    ...
    BOOL isLessThanDurationThreshold =
        preferredAdSlotTime < ([timeInMilliseconds floatValue] +
                               (kDurationThresholdInMinutes * 60 * 1000));
    BOOL isMoreThanCurrentTime =
        preferredAdSlotTime > [timeInMilliseconds floatValue];
...
- (NSString *)contentAtTime:(NSInteger)timeInMilliseconds {
    NSDictionary *time = [NSDictionary
        dictionaryWithObjectsAndKeys:@"", @"content",
                                     [NSNumber
                                         numberWithInteger:timeInMilliseconds],
                                     @"start_time", @"", @"end_time", nil];

    NSUInteger index =
        [self.segments indexOfObject:time
    ...
    NSInteger segmentStartTime = [[[self.segments objectAtIndex:index]
        valueForKeyPath:@"start_time"] integerValue];
    NSInteger segmentEndTime = [[[self.segments objectAtIndex:index]
        valueForKeyPath:@"end_time"] integerValue];
    ...
    if (segmentStartTime == timeInMilliseconds) { // case when exactly the start
                                                  // of a new segment, so use
                                                  // that segment
    } else {
        // use the segment just before but check for edgecase when only 1
        // segment and index is already 0
        if (index > 0)
            index--;
        segmentStartTime = [[[self.segments objectAtIndex:index]
            valueForKeyPath:@"start_time"] integerValue];
        segmentEndTime = [[[self.segments objectAtIndex:index]
            valueForKeyPath:@"end_time"] integerValue];
    }

    if (timeInMilliseconds >= segmentStartTime &&
        timeInMilliseconds < segmentEndTime) {
...
- (void)processSegments:(NSArray *)segments {
    ...
    for (int i = 0; i < segments.count; i++) {
        ...
        [times addObject:[NSValue valueWithCMTime:CMTimeMake(
                                                      [startTime integerValue],
                                                      1000)]];
        [times addObject:[NSValue
                             valueWithCMTime:CMTimeMake([endTime integerValue],
                                                        1000)]];

        if (i + 1 < segments.count) {
            NSNumber *gapStartTime = [segments[i] valueForKeyPath:@"end_time"];
            NSNumber *gapEndTime =
                [segments[i + 1] valueForKeyPath:@"start_time"];
            CGFloat gap = [gapEndTime floatValue] - [gapStartTime floatValue];
            if (gap >= kMinGapDurationInSeconds * 1000) {
                [adSlotTimes
                    addObject:[NSNumber
                                  numberWithFloat:[gapStartTime floatValue] +
                                                  gap / 2]];
            }
        }
    }
