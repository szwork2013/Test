...
- (void)tableView:(SLExpandableTableView *)tableView
    ...
    forRowAtIndexPathWhileAnimatingSection:(NSIndexPath *)indexPath;

- (void)tableView:(SLExpandableTableView *)tableView
    willExpandSection:(NSUInteger)section
...
@interface SLExpandableTableView
  ...
  @private
    ...
    id<UITableViewDataSource, SLExpandableTableViewDatasource> __weak
    _myDataSource;

    NSMutableDictionary *_expandableSectionsDictionary; // will store BOOLs for
                                                        // each section that is
                                                        // expandable
    NSMutableDictionary *_showingSectionsDictionary; // will store BOOLs for the
                                                     // sections state (nil: not
                                                     // expanded, 1: expanded)
    NSMutableDictionary *_downloadingSectionsDictionary; // will store BOOLs for
                                                         // the sections state
                                                         // (nil: not
                                                         // downloading, YES:
                                                         // downloading)
...
// discussion
// never use tableView.delegate/ tableView.dataSource as a getter. setDataSource
// will set _myDataSource, etc. so use these getters instead
@property(nonatomic, readonly, weak)
    id<SLExpandableTableViewDelegate> myDelegate;
@property(nonatomic, readonly, weak)
    id<SLExpandableTableViewDatasource> myDataSource;

@property(nonatomic, assign)
    NSInteger maximumRowCountToStillUseAnimationWhileExpanding;

@property(nonatomic, assign)
    BOOL onlyDisplayHeaderAndFooterViewIfTableViewIsNotEmpty;

@property(nonatomic, assign) UITableViewRowAnimation reloadAnimation;

// call tableView:needsToDownloadDataForExpandableSection: to make sure we can
// expand the section, otherwise through exception
