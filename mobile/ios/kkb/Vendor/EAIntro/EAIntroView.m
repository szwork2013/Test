...
- (void)finishIntroductionAndRemoveSelf {
    ...
    // Calling removeFromSuperview from scrollViewDidEndDecelerating: method
    // leads to crash on iOS versions < 7.0.
...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat *)xIndex {
    ...
    *xIndex += self.scrollView.frame.size.width;
    ...
    if (page.titleIconView) {
        ...
        rect1.origin.x =
            (self.scrollView.frame.size.width - rect1.size.width) / 2;
    ...
    if (page.title.length) {
        ...
        if ([page.title respondsToSelector:@selector(boundingRectWithSize:
                                                                  options:
                                                               attributes:
                                                                  context:)]) {
            NSAttributedString *attributedText = [[NSAttributedString alloc]
                initWithString:page.title
                    attributes:@{NSFontAttributeName : page.titleFont}];
            CGRect rect = [attributedText boundingRectWithSize:(CGSize) {
                self.scrollView.frame.size.width - 20, CGFLOAT_MAX
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            titleHeight =
                [page.title
                         sizeWithFont:page.titleFont
                    constrainedToSize:CGSizeMake(
                                          self.scrollView.frame.size.width - 20,
                                          CGFLOAT_MAX)
...
#pragma clang diagnostic pop
        ...
        CGRect titleLabelFrame =
            CGRectMake(10, self.frame.size.height - page.titlePositionY,
                       self.scrollView.frame.size.width - 20, titleHeight);
    ...
    if ([page.desc length]) {
        ...
        if (page.descWidth != 0) {
            descLabelFrame =
                CGRectMake((self.frame.size.width - page.descWidth) / 2,
                           self.frame.size.height - page.descPositionY,
                           page.descWidth, 500);
        } else {
            descLabelFrame =
                CGRectMake(0, self.frame.size.height - page.descPositionY,
                           self.scrollView.frame.size.width, 500);
        }

        UITextView *descLabel =
            [[UITextView alloc] initWithFrame:descLabelFrame];
    ...
    pageView.accessibilityLabel =
        [NSString stringWithFormat:@"intro_page_%lu", (unsigned long)
                                   [self.pages indexOfObject:page]];
...
- (void)appendCloseViewAtXIndex:(CGFloat *)xIndex {
    UIView *closeView = [[UIView alloc]
        initWithFrame:CGRectMake(*xIndex, 0, self.frame.size.width,
...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc]
        initWithFrame:CGRectMake(0, self.frame.size.height - self.pageControlY,
    ...
    self.skipButton = [[UIButton alloc]
        initWithFrame:CGRectMake(
                          self.scrollView.frame.size.width - 80,
                          self.pageControl.frame.origin.y -
                              ((30 - self.pageControl.frame.size.height) / 2),
                          80, 30)];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if ([(id)self.delegate respondsToSelector:@selector(intro:
                                                  pageStartScrolling:
                                                           withIndex:)]) {
...
- (void)notifyDelegateWithPreviousPage:(NSInteger)previousPageIndex
                        andCurrentPage:(NSInteger)currentPageIndex {
...
- (void)setTitleView:(UIView *)titleView {
    ...
    _titleView.frame =
        CGRectMake((self.frame.size.width - _titleView.frame.size.width) / 2,
                   self.titleViewY, _titleView.frame.size.width,
...
- (void)setPageControlY:(CGFloat)pageControlY {
    ...
    self.pageControl.frame = CGRectMake(0, 460, self.frame.size.width, 20);
...
- (void)addMotionEffectsOnBg {
    ...
    CGRect parallaxFrame = CGRectMake(
        ...
        self.frame.size.width + (self.motionEffectsRelativeValue * 2),
        self.frame.size.height + (self.motionEffectsRelativeValue * 2));
    ...
    verticalMotionEffect = [[UIInterpolatingMotionEffect alloc]
    ...
    UIInterpolatingMotionEffect *
    horizontalMotionEffect = [[UIInterpolatingMotionEffect alloc]
...
- (void)removeMotionEffectsOnBg {
    ...
    [UIView
        animateWithDuration:0.5f
                 animations:^{
                     [self.pageBgFront
                         removeMotionEffect:self.pageBgFront.motionEffects[0]];
                     [self.pageBgBack
                         removeMotionEffect:self.pageBgBack.motionEffects[0]];
                     [self.bgImageView
                         removeMotionEffect:self.bgImageView.motionEffects[0]];
                 }];
}
