...
+ (void)photoPickerWithTitle:(NSString *)title
    ...
    if ([UIImagePickerController
        ...
        [actionSheet
            addButtonWithTitle:NSLocalizedString(@"Photo library", @"")];
        cancelButtonIndex++;
    }
...
+ (void)imagePickerController:(UIImagePickerController *)picker
    ...
    if (!editedImage)
        editedImage =
            (UIImage *)[info valueForKey:UIImagePickerControllerOriginalImage];
    ...
    if (buttonIndex == [actionSheet cancelButtonIndex]) {
        ...
        if (actionSheet.tag == kPhotoActionSheetTag) {
            ...
            picker.delegate = (id<UINavigationControllerDelegate,
                                  UIImagePickerControllerDelegate>)[self class];
...
- (void)setLargeTitle:(BOOL)largeTitle {
    if (largeTitle) {
        ...
        if (label) {
            ...
            [label setFont:font];
            [label setFrame:CGRectMake(0, label.frame.origin.y,
