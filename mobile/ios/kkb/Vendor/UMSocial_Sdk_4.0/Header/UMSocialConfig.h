...
/**
 ...
 所在UINavigationController的navigationItem，可以改变相应的标题
 ...
 */
typedef void (^UMTableViewCellConfig)(UITableViewCell *cell,
                                      UMSViewControllerType viewControllerType);
...
/**
 ...
 @param platformNames
 在`UMSocialSnsPlatformManager.h`定义的UMShareToSina、UMShareToTencent、UMShareToQzone、UMShareToRenren、UMShareToDouban、UMShareToEmail、UMShareToSms组成的NSArray
 */
...
@param interfaceOrientations 一个bit map（位掩码），ios
6定义的`UIInterfaceOrientationMask`
*/
...
/**
 ...
 例如下面写法
 ```
 [UMSocialConfig setShareGridViewTheme:^(CGContextRef ref, UIImageView
*backgroundView,UILabel *label){
 //改变线颜色和线宽
...
initWithFrame:backgroundView.frame];
 imageView.image = [UIImage imageNamed:@"share_bg"];
...
/**
 ...
 imageNamed:@"UMSocialSDKResourcesNew.bundle/OtherTheme/UMS_nav_bar_bg"];

     if ([bar respondsToSelector:@selector(setBackgroundImage:forBarMetrics:)])
 {
         [bar setBackgroundImage:backgroundImage
 ...
 */
+ (void)setFinishToastIsHidden:(BOOL)isHidden
                      position:(UMSocialiToastPosition)toastPosition;

/**
 设置官方微博账号,设置之后可以在授权页面有关注微博的选项，默认勾选，授权之后用户即关注官方微博，仅支持新浪微博和腾讯微博

 @param weiboUids
 腾讯微博和新浪微博的key分别是`UMShareToSina`和`UMShareToTenc`,值分别是官方微博的uid,例如`[UMSocialConfig
 setFollowWeiboUids:[NSDictionary
 dictionaryWithObjectsAndKeys:@"yourSinaUid",UMShareToSina,nil]];`
 */
...
/** deprecated API, Use ''[UMSocialSinaHandler
 openSSOWithRedirectURL:@"http://sns.whalecloud.com/sina2/callback"];''

 设置是否支持新浪微博SSO，默认不支持
 @param supportSinaSSO 设置是否支持新浪微博SSO
 @param appRedirectUrl
 设置授权回调地址，此授权回调地址必须和你在新浪应用后台填写的回调地址一致，否则不能授权
        如果在新浪微博后台绑定我们的回调地址“http://sns.whalecloud.com/sina2/callback”，这里可以传nil

 */
//+ (void)setSupportSinaSSO:(BOOL)supportSinaSSO appRedirectUrl:(NSString
//*)appRedirectUrl;

/** deprecated API, Use ''[UMSocialQQHandler shareToQQWithAppId:@"100424468"
 ...
 @param url
 手机QQ图文分享web类型，用到的url地址，如果传nil，默认使用http://www.umeng.com/social
 @param classes 载入QQ互联 SDK，用到的两个类
 */

//+ (void)setQQAppId:(NSString *)appId url:(NSString *)url
//importClasses:(NSArray *)classes;
...
//+ (void)setSupportQzoneSSO:(BOOL)supportQzoneSSO importClasses:(NSArray
//*)classes;
...
//+ (void)setShareQzoneWithQQSDK:(BOOL)useQQSDK url:(NSString *)urlString
//importClasses:(NSArray *)classes;
