...
+ (NSString *)md5StringForData:(NSData*)data{
    ...
    CC_MD5(data.bytes, (unsigned int)data.length, r);
...
+ (NSString *)md5StringForString:(NSString*)str{
    ...
    unsigned char r[CC_MD5_DIGEST_LENGTH];
    CC_MD5(str1, (unsigned int)strlen(str1), r);
    return [NSString stringWithFormat:@"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10], r[11], r[12], r[13], r[14], r[15]];
}
...
+(NSString *)getVideosFolderPathWithBasepath:(NSString *)name
{
    NSString *pathstr = [[self class]getDocumentPath];
    pathstr = [pathstr stringByAppendingPathComponent:name];
    pathstr =  [pathstr stringByAppendingPathComponent:@"videos"];
    NSFileManager *fileManager=[NSFileManager defaultManager];
    NSError *error;
    if(![fileManager fileExistsAtPath:pathstr])
    {
