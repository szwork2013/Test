...
@implementation AnnouncementOperator
...
- (id)init
{
    if (self = [super init])
    {
        self.moduleType = KAIKEBA_MODULE_ANNOUNCEMENT;
...
//获取通告
- (void)requestAnnouncements:(id)delegate token:(NSString *)token courseId:(NSString *)courseId
{
    NSString *jsonUrl = [NSString stringWithFormat:@"%@courses/%@/discussion_topics?only_announcements=true", API_HOST, courseId];
    [self getJSONFromUrl:delegate command:HTTP_CMD_COURSE_ANNOUNCEMENTS jsonUrl:jsonUrl token:token];
}
//回复人员讨论
...
//    NSLog(@"%@",jsonUrl);
    [self getJSONFromUrl:delegate command:HTTP_CMD_COURSE_ANNOUNCEMENTS_ALLTOPICENTRY jsonUrl:jsonUrl token:token];
}
...
//通告
- (void)parseAnnouncements:(NSString *)jsonDatas
{
    NSArray *array = [jsonDatas objectFromJSONString];
    [self.discussionTopic.announcementsItemArray removeAllObjects];
    for(NSDictionary *dic in array)
    {
        ...
        [self.discussionTopic.announcementsItemArray addObject:item];
    }
}
...
//                NSLog(@"===%@",[dic objectForKey:@"message"]);
    ...
    NSLog(@"numIN=====%lu",(unsigned long)[self.discussionTopic.allTopicEntry.views count]);
}
...
- (void)requestFinished:(id)subDelegate cmd:(NSString *)cmd jsonDatas:(NSString *)jsonDatas url:(NSString *)url
{
    ...
    {
        [self parseAnnouncements:jsonDatas];
    }
    else if ([cmd compare:HTTP_CMD_COURSE_ANNOUNCEMENTS_ALLTOPICENTRY] == NSOrderedSame)
    {
